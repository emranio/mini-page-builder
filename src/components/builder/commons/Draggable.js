...
import { BlockItemView } from '../panels/partials/'


const DragPreview = React.forwardRef(({ type, isDragging }, ref) => (
    <div
        ref={ref}
        style={{
            background: "none",
...
const Draggable = ({ id, type, parentId, children }) => {
    ...
    // the drag preview
    useEffect(() => {
        if (previewRef.current && ref.current) {
    ...
    return (
        <>
            <DragPreview ref={previewRef} type={type} isDragging={isDragging} />
